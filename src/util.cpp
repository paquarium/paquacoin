...
const char * const BITCOIN_CONF_FILENAME = "paquacoin.conf";
const char * const BITCOIN_PID_FILENAME = "paquacoin.pid";
...
#else
    const char* pszModule = "paquacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Paquacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Paquacoin";
#else
    // Unix
    return pathRet / ".paquacoin";
#endif
#endif
}
